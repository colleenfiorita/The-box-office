...
function getBrandFromTicket(ticket: Ticket): string {
  if (ticket.clientCompany && ticket.clientCompany !== "Internal" && ticket.clientCompany !== "Unknown") {
    return ticket.clientCompany;
  }
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border/50 bg-card/50 backdrop-blur-sm">
      ...
      <div className="divide-y divide-border/30">
        {sorted.map((ticket, i) => {
          const brand = getBrandFromTicket(ticket);
          return (
            <motion.div
              key={ticket.id}
            ...
            >
              <button
                ...
                className={cn(
                  "w-full grid grid-cols-[1fr_120px_110px_90px_80px_60px_80px] gap-2 px-4 py-3 text-left text-sm transition-colors hover:bg-accent/30",
                  ...
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    ...
                    <div>
                      ...
                      {ticket.clientCompany && ticket.clientCompany !== brand && (
                        <p className="text-muted-foreground mt-0.5 text-[10px]">{ticket.clientCompany}</p>
                      )}
                    </div>
                    ...
                    <div>
                      <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Messages</p>
                      <p className="font-mono text-foreground">{ticket.messageCount}</p>
                    </div>
                    {/* Open in Gmail link */}
                    <div className="col-span-3 pt-2 border-t border-border/20">
                      <a
